...
patrones_criticos = [
    ...
    r'hecho de importancia', r'deuda', r'bonos', r'soberano', r'investment grade'
]
patron_riesgo = re.compile('|'.join(patrones_criticos), re.IGNORECASE)
patron_basura = re.compile(r'television|televisión|conductor|primiciasya|casella|espectaculo|futbol|farandula', re.IGNORECASE)
            ...
            for emisor, prod in mapping_emisores.items():
                ...
                if raiz_emisor in linea.lower() or nemonico in linea.lower():
                    ...
                    datos_centralizados[prod][emisor]["noticias"].append({
                        "titulo": titulo_limpio, "link": portal["url"], "fuente": portal["fuente"], "fecha": "Hoy", "prioritaria": es_prioritaria
                    })
    except Exception as e:
        pass
...
for idx, (emisor, prod) in enumerate(mapping_emisores.items()):
    try:
        ...
        if is_soberano:
            ...
            query_bvl = f'("{emisor}"%20OR%20"{cod_bvl}")%20(Moody%27s%20OR%20Downgrade%20OR%20Sindicado%20OR%20SMV%20OR%20BVL%20OR%20Outlook%20OR%20Suscripcion)'
        ...
        for url in urls_red:
            try:
                ...
                for item_content in items_raw[:5]:
                    ...
                    datos_centralizados[prod][emisor]["noticias"].append({
                        "titulo": titulo_final, "link": link, "fuente": fuente, "fecha": fecha_limpia, "prioritaria": es_prioritaria
                    })
            except:
                pass
    except:
...
# FASE 4: MAQUETADO DE HTML SEGURO (Sin comillas triples para evitar SyntaxError al copiar)
html_lines = []
...
html_lines.append("<title>Monitoreo de Noticias del Portafolio</title>")
html_lines.append("<script src='https://cdn.tailwindcss.com'></script>")
html_lines.append("</head>")
html_lines.append("<body class='bg-gray-950 text-gray-100 min-h-screen font-sans'>")
html_lines.append("<div class='max-w-7xl mx-auto px-4 py-8'>")

html_lines.append("<header class='border-b border-gray-800 pb-6 mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-4'>")
html_lines.append("<div>")
html_lines.append("<h1 class='text-3xl font-extrabold bg-gradient-to-r from-blue-400 via-indigo-400 to-emerald-400 bg-clip-text text-transparent'>Monitoreo de Noticias del Portafolio</h1>")
html_lines.append("<p class='text-gray-400 text-sm mt-1'>Filtro de Crédito Avanzado • Sincronización Estricta de tu Portafolio de Inversión</p>")
html_lines.append("</div>")
html_lines.append("<div class='bg-red-950/40 border border-red-500/30 px-4 py-2 rounded-xl text-xs font-mono text-red-400 flex items-center gap-2'>")
html_lines.append("<span class='w-2 h-2 rounded-full bg-red-500 animate-pulse'></span> Terminal Blindada Activa")
html_lines.append("</div>")
...
for producto in orden_productos:
    ...
    if not any(contenido["noticias"] for emisor, contenido in emisores_del_producto.items()):
        continue
    ...
    html_lines.append("<section class='mb-12'>")
    html_lines.append("<div class='flex items-center gap-3 mb-6 border-b border-gray-800 pb-2'>")
    html_lines.append(f"<h2 class='text-xl font-bold text-white'>{producto}</h2>")
    html_lines.append(f"<span class='border text-xs px-2.5 py-0.5 rounded-md font-medium {badge_color}'>Segmento</span>")
    html_lines.append("</div>")
    html_lines.append("<div class='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>")
